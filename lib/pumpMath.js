...
(function (global) {
  ...
  function refModel(pump) {
    if (pump.useCatalog && Array.isArray(pump.catalog)) {
      const rows = pump.catalog.filter(r => Number.isFinite(r.q) && r.q >= 0 && Number.isFinite(r.h));
      const positiveRows = rows.filter(r => r.q > 0);
      if (rows.length >= 2 && positiveRows.length > 0) {
        const Hco = fitPoly(rows.map(r => ({ x: r.q, y: r.h })));
        const etaRows = rows.filter(r => r.q > 0 && Number.isFinite(r.eta) && r.eta > 0);
        const Eco = etaRows.length >= 2 ? fitPoly(etaRows.map(r => ({ x: r.q, y: r.eta }))) : null;
        const npRows = rows.filter(r => r.q >= 0 && Number.isFinite(r.npshr) && r.npshr > 0);
        ...
        else if (etaRows.length) QbepRef = etaRows.reduce((m, r) => (r.eta || 0) > (m.eta || 0) ? r : m, etaRows[0]).q;
        else QbepRef = pump.Qb || positiveRows[positiveRows.length - 1].q * 0.8;
